...
async def _run_via_relay(file_size: int) -> None:
    """Spin up a local relay, have sender+receiver connect through it."""
    from lentel.server.relay import _Relay
    ...
    with tempfile.TemporaryDirectory() as tdir:
        ...
        # Start relay on a random free UDP port on 127.0.0.1
        loop = asyncio.get_event_loop()
        relay_port = _reserve_port()
        relay_transport, _relay_proto = await loop.create_datagram_endpoint(
            _Relay, local_addr=("127.0.0.1", relay_port),
        )

        try:
            relay_url = f"127.0.0.1:{relay_port}"

            # Sender uses relay mode; ticket will be generated by send_file.
            ...
            sender = asyncio.create_task(
                send_file(
                    src, parallel=4, wait_timeout=30.0,
                    relay=relay_url, on_ticket=on_ticket,
                ),
            )
            ...
            assert "t" in ticket_holder, "sender never produced a ticket"

            receiver = asyncio.create_task(
                recv_file(ticket_holder["t"], dest_dir=dst, parallel=4),
            )
...
def test_relay_small():
    asyncio.run(_run_via_relay(1024))


def test_relay_medium():
    asyncio.run(_run_via_relay(500_000))


def test_relay_empty():
    asyncio.run(_run_via_relay(0))
